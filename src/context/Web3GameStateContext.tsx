...
export function Web3GameStateProvider({ children }: { children: React.ReactNode }) {
  ...
  const [koiBalance, setKoiBalance] = useState<number>(350);
  const [pearlBalance, setPearlBalance] = useState<number>(120);
  ...
  const [lastMiningClaim, setLastMiningClaim] = useState<number>(1700000000000);
  ...
  const [referralEarnings, setReferralEarnings] = useState<number>(42.5);

  // Load from localStorage post-hydration
  useEffect(() => {
    const timer = setTimeout(() => {
      try {
        ...
        if (saved) {
          ...
          if (parsed.referralEarnings !== undefined) setReferralEarnings(parsed.referralEarnings);
        } else {
          setLastMiningClaim(Date.now() - 3600 * 1000 * 4);
        }
      } catch (e) {
